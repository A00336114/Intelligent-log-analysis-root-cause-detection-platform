...
# Standard categories we expect to see
...
class FeatureEngineer:
    ...
    def build_feature_matrix(self, df: pd.DataFrame, fit: bool = False) -> np.ndarray:
        ...
        if fit:
            ...
            # Try to load, if not exist, fit on the incoming data
    ...
    def extract_features_single(self, record: Dict[str, Any]) -> np.ndarray:
        ...
        # 2. is_error (logLevel is ERROR/FATAL)
        ...
        if timestamp_str:
            try:
                # Clean up ISO format offset (e.g. +01:00 or Z)
                # Just extract the hour part if possible, or parse with pandas
